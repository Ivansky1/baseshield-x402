...
The stale uv.lock was removed after adding x402; it did not contain the payment
SDK. requirements.txt (and pyproject.toml) pin direct deployment dependencies.
Regenerate a full lock with `uv lock` when registry access is available before
using `uv sync --frozen`. A clean install of these pins was not verified locally
because the registry connection timed out; the current test run used installed
Python 3.14 dependencies and the official x402 2.23.0 source checkout. The added
Python 3.12 CI workflow has not yet run on the hosting service.
